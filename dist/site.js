...
/* Busca e filtros dos projetos */
const projectSearch=document.querySelector('#project-search');
if(projectSearch){const projectItems=[...document.querySelectorAll('.project-item[data-project-status]')],note=document.querySelector('#project-results-note');let scope='all';const norm=v=>String(v||'').toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');const renderProjects=()=>{const term=norm(projectSearch.value.trim());let shown=0;projectItems.forEach(item=>{const matchScope=scope==='all'||item.dataset.projectStatus===scope;const matchTerm=!term||norm(item.textContent).includes(term);const show=matchScope&&matchTerm;item.classList.toggle('is-filtered',!show);if(show)shown++;});if(note)note.textContent='Exibindo '+shown+' '+(shown===1?'projeto':'projetos');};document.querySelectorAll('.project-filter-button').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('.project-filter-button').forEach(b=>b.classList.remove('is-active'));button.classList.add('is-active');scope=button.dataset.projectFilter;renderProjects();}));projectSearch.addEventListener('input',renderProjects);renderProjects();}

/* Buscas independentes por frente de projetos */
document.querySelectorAll('.section-project-search-input').forEach(input=>{input.addEventListener('input',()=>{const wrap=input.closest('.project-inline-list');const term=String(input.value||'').toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');wrap.querySelectorAll('.project-item[data-project-status]').forEach(item=>{const text=item.textContent.toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');item.classList.toggle('is-filtered',Boolean(term&&!text.includes(term)));});});});

/* Paginação e filtros no padrão de Publicações */
if(document.querySelector('#project-page-size')){const items=[...document.querySelectorAll('.project-item[data-project-status]')],search=document.querySelector('#project-search'),note=document.querySelector('#project-results-note'),sizeSelect=document.querySelector('#project-page-size');let scope='all',page=1,size=Number(sizeSelect.value);const norm=v=>String(v||'').toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');const render=()=>{const term=norm(search.value.trim()),filtered=items.filter(i=>(scope==='all'||i.dataset.projectStatus===scope)&&(!term||norm(i.textContent).includes(term))),pages=Math.max(1,Math.ceil(filtered.length/size));page=Math.min(page,pages);const first=(page-1)*size,visible=new Set(filtered.slice(first,first+size));items.forEach(i=>i.classList.toggle('is-filtered',!visible.has(i)));note.textContent=filtered.length?'Exibindo '+(first+1)+'–'+Math.min(first+size,filtered.length)+' de '+filtered.length+' projetos':'Nenhum projeto encontrado.';let nav=document.querySelector('#project-pagination');if(!nav){nav=document.createElement('nav');nav.id='project-pagination';nav.className='conference-pagination';nav.setAttribute('aria-label','Paginação dos projetos');document.querySelector('.project-tools').after(nav);}nav.innerHTML=Array.from({length:pages},(_,i)=>'<button type="button" data-project-page="'+(i+1)+'" class="'+(page===i+1?'is-active':'')+'">'+(i+1)+'</button>').join('');nav.querySelectorAll('button').forEach(b=>b.onclick=()=>{page=Number(b.dataset.projectPage);render();});};document.querySelectorAll('.project-filter-button').forEach(b=>b.onclick=()=>{document.querySelectorAll('.project-filter-button').forEach(x=>x.classList.remove('is-active'));b.classList.add('is-active');scope=b.dataset.projectFilter;page=1;render();});search.oninput=()=>{page=1;render();};sizeSelect.onchange=()=>{size=Number(sizeSelect.value);page=1;render();};render();}

/* Controles independentes em cada seção */
document.querySelectorAll('.project-inline-list[data-project-section]').forEach(section=>{const input=section.querySelector('.section-project-search-input'),note=section.querySelector('.section-project-results'),select=section.querySelector('.section-project-page-size'),items=[...section.querySelectorAll('.project-item[data-project-status]')];let scope='all',page=1,size=Number(select.value);const norm=v=>String(v||'').toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');const render=()=>{const term=norm(input.value.trim()),filtered=items.filter(i=>(scope==='all'||i.dataset.projectStatus===scope)&&(!term||norm(i.textContent).includes(term))),pages=Math.max(1,Math.ceil(filtered.length/size));page=Math.min(page,pages);const first=(page-1)*size,visible=new Set(filtered.slice(first,first+size));items.forEach(i=>i.classList.toggle('is-filtered',!visible.has(i)));note.textContent=filtered.length?'Exibindo '+(first+1)+'–'+Math.min(first+size,filtered.length)+' de '+filtered.length+' projetos':'Nenhum projeto encontrado.';let nav=section.querySelector('.section-project-pagination');if(!nav){nav=document.createElement('nav');nav.className='section-project-pagination conference-pagination';nav.setAttribute('aria-label','Paginação da seção');section.append(nav);}nav.innerHTML='<button type="button" data-page="'+(page-1)+'" '+(page===1?'disabled':'')+'>Anterior</button>'+Array.from({length:pages},(_,i)=>'<button type="button" class="'+(page===i+1?'is-active':'')+'" data-page="'+(i+1)+'">'+(i+1)+'</button>').join('')+'<button type="button" data-page="'+(page+1)+'" '+(page===pages?'disabled':'')+'>Próxima</button>';nav.querySelectorAll('button[data-page]').forEach(b=>b.onclick=()=>{page=Number(b.dataset.page);render();});};section.querySelectorAll('.section-project-filter').forEach(b=>b.onclick=()=>{section.querySelectorAll('.section-project-filter').forEach(x=>x.classList.remove('is-active'));b.classList.add('is-active');scope=b.dataset.filter;page=1;render();});input.oninput=()=>{page=1;render();};select.onchange=()=>{size=Number(select.value);page=1;render();};render();});
