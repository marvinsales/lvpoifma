...
if (journalList) {
  const publications = [
    ...
    { year: 2026, type: 'Artigo científico', cover: 'IJMS', journal: 'International Journal of Mechanical Sciences', volume: 'v. 244', pages: '113747', title: 'Bandgap optimization in locally resonant metamaterial plates: A comparative study of five lattice geometries for low-frequency wave attenuation', authors: 'Ferreira, A. H. R.; Miranda Jr., E. J. P.; Goto, A. M.; Dos Santos, J. M. C.', link: 'https://papers.ssrn.com/', page: 'publicacoes-ijmecs-113747.html', citations: [{ label: 'Scopus', value: 2, link: 'https://www.scopus.com/' }] },
    { year: 2026, type: 'Artigo científico', cover: 'JBSMSE', journal: 'Journal of the Brazilian Society of Mechanical Sciences and Engineering', volume: 'v. 48', pages: '309', title: 'Wave and vibration attenuation via full band gaps in 3D periodic sandwich metapanels', authors: 'Sales, M. B. M.; Araujo, B. C. C.; Santos, M. C. P.; Pereira, F. N.; Miranda Jr., E. J. P.', link: 'https://link.springer.com/', page: 'publicacoes-jbsmse-48-309.html', citations: [] },
    { year: 2025, type: 'Artigo científico', cover: 'REREM', journal: 'Revista de Estruturas e Materiais', volume: 'v. 18', pages: 'e18112', title: 'Microstructural, mechanical, and acoustic analyses of concrete with EPS and silica fume', authors: 'Silva, V. G. S.; Sandes Filho, C. G.; Angelin, A. F. P.; Paiva, R. E. M.; Palma, S. S. A.; Gonçalves, R.; Lintz, R. C. C.; Gachet, L. A.; Miranda Jr., E. J. P.', link: 'https://www.scielo.br/', page: 'publicacoes-rerem-e18112.html', citations: [] },
    ...
    { year: 2025, type: 'Artigo científico', cover: 'IJMSE', journal: 'International Journal of Mechanical Sciences', volume: 'v. 293', pages: '110125', title: 'Wave and vibration attenuation in graded elastic metamaterial beams with local resonators', authors: 'Gomes, C. B. F.; Dos Santos, M. C. P.; Araujo, B. C. C.; Pereira, F. N.; Nobrega, E. D.; Dos Santos, J. M. C.; Miranda Jr., E. J. P.; Sinatora, A.', link: 'https://www.sciencedirect.com/journal/international-journal-of-mechanical-sciences', page: 'publicacoes-ijmecs-110125.html', citations: [{ label: 'Web of Science', value: 2, link: 'https://www.webofscience.com/' }, { label: 'Scopus', value: 22, link: 'https://www.scopus.com/' }] }
  ];
  ...
  const render = () => {
    ...
    journalList.innerHTML = filtered.length ? filtered.map(item => `<article class="publication publication-card"><div class="journal-cover cover-${item.cover.toLowerCase()}"><span>${item.cover}</span><small>${item.journal}</small><b>${item.year}</b></div><div class="publication-body"><div class="publication-topline"><p class="meta">${item.type} · ${item.year}</p><span class="publication-index">${String(item.year).slice(-2)}</span></div><h3><a class="publication-title-link" href="${item.page}">${item.title}</a></h3><p class="authors">${item.authors}</p><p class="journal"><em>${item.journal}</em> · ${item.volume} · ${item.pages}</p><div class="publication-actions"><a href="${item.page}">Detalhes <span>↗</span></a><a href="${item.link}" target="_blank" rel="noopener">Ver publicação <span>↗</span></a>${item.doi ? `<a href="https://doi.org/${item.doi}" target="_blank" rel="noopener">DOI <span>↗</span></a>` : ''}${item.citations.map(c => `<a class="citation-badge" href="${c.link}" target="_blank" rel="noopener"><strong>${c.value}</strong> ${c.label}</a>`).join('')}</div></div></article>`).join('') : '<div class="empty-state"><strong>Nenhuma publicação encontrada.</strong><span>Tente outro termo ou remova o filtro de ano.</span></div>';
  };
